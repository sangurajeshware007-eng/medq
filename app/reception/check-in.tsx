...
import {
  ChevronRight, Building2, Search, CheckCircle, Clock,
  User, Stethoscope, X, RefreshCw,
...
export default function CheckInAppointmentScreen() {
  ...
  const load = React.useCallback(async () => {
    if (!hospital) { setBookings([]); return; }
    setLoading(true);
    try {
  ...
  async function handleCheckIn(b: ReceptionBookingItem) {
    Alert.alert(
      'Check in patient',
      `Mark ${b.patientName} as present for ${b.slotStart}?`,
      [
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {hospital && <Text style={styles.headerSub} numberOfLines={1}>{hospital.hospitalName}</Text>}
        </View>
      ...
      {!loadingHospitals && hospitals.length > 1 && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.hospitalsRow}>
          {hospitals.map((h) => (
            <TouchableOpacity
              key={h.hospitalId}
              onPress={() => setHospital(h)}
              style={[styles.hospitalChip, hospital?.hospitalId === h.hospitalId && styles.hospitalChipActive]}
            >
              <Building2 size={13} color={hospital?.hospitalId === h.hospitalId ? Colors.white : Colors.primary} strokeWidth={2.5} />
              <Text style={[styles.hospitalChipText, hospital?.hospitalId === h.hospitalId && { color: Colors.white }]}>
                {h.hospitalName}
              </Text>
            </TouchableOpacity>
          ))}
            ...
            <View key={b.bookingId} style={[styles.card, b.checkedIn && styles.cardCheckedIn]}>
              ...
              <View style={styles.cardMid}>
                <View style={styles.cardName}>
                  ...
                  <Text style={styles.cardNameText} numberOfLines={1}>{b.patientName}</Text>
                  {b.source === 'WALK_IN' && (
                ...
                <Text style={styles.cardSub} numberOfLines={1}>{b.patientPhone}</Text>
                <View style={styles.cardDoctor}>
                  <Stethoscope size={11} color={Colors.textSecondary} strokeWidth={2} />
                  <Text style={styles.cardDoctorText} numberOfLines={1}>{b.doctorName}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  hospitalsRow: { paddingHorizontal: 16, paddingVertical: 8, gap: 8, backgroundColor: Colors.white },
  hospitalChip: {
  ...
  datesRow: { paddingHorizontal: 16, paddingVertical: 10, gap: 8, backgroundColor: Colors.white, borderBottomWidth: 1, borderBottomColor: Colors.borderLight },
  dateChip: {
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10,
    borderWidth: 1.5, borderColor: Colors.borderLight,
    backgroundColor: Colors.white, alignItems: 'center', minWidth: 60,
  },
  ...
  list: { flex: 1 },
  listContent: { padding: 16, paddingTop: 4, paddingBottom: 40, gap: 10 },
  ...
  token: {
    minWidth: 56, paddingHorizontal: 8, paddingVertical: 6, borderRadius: 10,
  ...
  cardRef: { fontSize: 10, color: Colors.textLight, fontWeight: '600', letterSpacing: 0.5, marginTop: 2 },
