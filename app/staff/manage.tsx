...
export default function ManageStaffScreen() {
  ...
  const { user } = useAuth();
  const { data: hospitalData } = useHospitalManagerDashboard({
  ...
  const [inviteRole, setInviteRole] = useState<'HOSPITAL_RECEPTIONIST' | 'HOSPITAL_MANAGER'>(
  ...
  async function handleReject(id: string) {
    Alert.alert('Reject Request', 'Are you sure you want to reject this request?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await staffService.managerReject(id);
            load();
          } catch (e: unknown) {
  ...
  async function handleRevoke(id: string, name: string | null) {
    Alert.alert('Revoke Access', `Remove ${name ?? 'this person'} from your staff?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await staffService.managerRevoke(id);
            load();
          } catch (e: unknown) {
            Alert.alert('Error', getApiErrorMessage(e, 'Failed.'));
          }
            ...
            <View key={s.id} style={styles.card}>
              <View style={styles.cardLeft}>
                <Text style={styles.cardName}>
                  {s.userName ?? s.phonePending ?? s.userPhone ?? '—'}
                </Text>
                ...
                <View
                  style={[
                    styles.roleBadge,
                    s.role === 'HOSPITAL_MANAGER' && styles.roleBadgeManager,
                  ]}
                >
                  <Text
                    style={[
                      styles.roleText,
                      s.role === 'HOSPITAL_MANAGER' && styles.roleTextManager,
                    ]}
                  >
                    {s.role.replace(/_/g, ' ')}
                  </Text>
              ...
              <View style={styles.cardActions}>
                ...
                {tab === 'active' && (
                  <TouchableOpacity
                    ...
                    onPress={() => handleRevoke(s.id, s.userName)}
                  >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            {(['HOSPITAL_RECEPTIONIST', 'HOSPITAL_MANAGER'] as const).map((r) => (
              ...
              >
                <Text
                  style={[styles.roleOptionText, inviteRole === r && { color: Colors.primary }]}
                >
                  {r.replace(/_/g, ' ')}
                </Text>
                {inviteRole === r && (
...
const styles = StyleSheet.create({
  ...
  list: { flex: 1 },
  listContent: { ...contentColumn, padding: 16, gap: 10 },
